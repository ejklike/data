...
def get_data(query_url):
    ...
    """
    def _retrieve_data(query_url, timeout=100, sleep_time=2):
...
class NearbySearchCaller(object):
    ...
    def run_recursive_query(self, lat, lng, radius=500):
        ...
        radius = int(radius * 1.414 /100 + 1) * 100
        ...
        for i, (lat, lng) in enumerate(zip(this_lat_list, this_lng_list), 1):
            query_result, status = self.run_query(lat, lng, radius)
            print(i, lat, lng, radius, status)
            _result += query_result
        _result = remove_duplicate(_result)
        return _result

    def run_query(self, lat, lng, radius=500):
